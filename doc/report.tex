...
\end{equation*} 
in the last step we used $\|z\|_2=1$ on $\Omega$. Thus the calculation of the Gaussian width boils down to bounding
\begin{equation}
w(\Omega) = \EE_g \left[ \sup_{z}\, \sprod{g, z} \:\:  \text{ s.t. } \|z\|_1 \leq 2\sqrt{k} \right] \leq \E_g \left[\sup_z \norm{g}_\infty \norm{z}_1 \text{ s.t. } \|z\|_1 \leq 2\sqrt{k}\right].
\end{equation}
Then we could use Theorem \ref{thm:UniqueRecovery} and obtain $m\geq  w(\Omega)^2 + 1$.\\

Using Hoelder's inequality the remaining work is to bound $\EE_g \|g\|_{\infty}$. 
\end{proof}
...
take into account the block structure of our measurements. However,
...
$\O(k\log(n/k))$. Intuitively, the first factor specifies the number
...
support, we can use a technique like \emph{model-based compressed
  sensing} \cite{model_sensing} to reduce this second factor. Let
$m_k$ be the number of possible support configurations with sparsity
$k$, then with a model-based approach we need on the order or
$\O(k\log(m_k))$ measurements. For $m_k = {n\choose k}\approx
(ne/k)^k$, we recover the compressed sensing result. In our case, if
we have $b$ blocks of size $\nu$ each, each with sparsity $\kappa$
