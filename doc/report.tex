...
\end{inparaenum}
\end{theorem}
However these are not checkable in polynomial time, see \cite{DonohoCheckable}. For random matrices with Gaussian entries this holds with high probability. Similar conditions which are easier to check is the mutual coherence, see e.g. \cite{Fuchs}.

Cardinality minimization is known to be a combinatorically hard problem. Therefore in the past decade there has been a lot of work done linked to Compressed Sensing
