...
However this program is not solvable in polynomial time because it is combinatorically hard problem.Therefore in the past decade there has been a lot of work done linked to Compressed Sensing in order to come up with convex relaxation of the $\ell^0$ norm which is basically the  and to find conditions for which solving the approximation/relaxation work.s

 For random matrices with Gaussian entries this holds with high probability. Similar conditions which (Restricted) Nullspace property/restricted eigenvalue property and restriced isometry property. For deterministic matrices these are not checkable in polynomial time, see \cite{DonohoCheckable}.  are easier to check is the mutual coherence, see e.g. \cite{Fuchs}.

Cardinality minimization is known to be a combinatorically hard problem. 
...
\begin{figure}
  \subfloat[][Small road network with weights]{\includegraphics[width=0.4\textwidth]{figures/small-graph.pdf}}
  \qquad
  \subfloat[][Flows for the small network]{\includegraphics[width=0.4\textwidth]{figures/small-flow.pdf}}
