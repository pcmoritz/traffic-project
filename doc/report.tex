...
In this paper, we formulate the traffic demand estimation problem as a nonnegative least squares problem with linear constraints and investigate the use of sparse flow measurements to recover the sparse route splits among routes starting from each origin\footnote{Based on a problem formulation by Alexander Kurzhanskiy (Berkeley, EECS) on the subject of "Calculating route splits from sparse link flow data"}. The sparsity assumption is intuitive for the traffic setting because it is natural for the flows out of a particular origin in a city to be concentrated in a handful other locations, rather than uniformly spread out over all other locations. There may of course be a small amount of flow between all points, and we may model that small perturbation as additive noise in the usual compressed sensing methodology. For instance, a highway during the morning commute hours may have most of its flow concentrated on routes that flow towards key business centers of a nearby city.
...
\begin{equation*}
\min_{1^Tx = 1, y\geq 0, 1^Ty =1} x_i : A_S x = A_{S^c} y \qquad>0,
\end{equation*}
the vector $x^*$ can be recovered as the unique solution of the linear program
...
\end{align}
implies that the support of the solution of \eqref{i-lp} coincides with the support of the original $x^*$. The dual Lagrangian of \eqref{min-cardinality} is
\begin{align*}
%&\min_{w,\lambda} w^\T b + \lambda +  \max_{x\geq 0}\,(e_i- A^\T w - \lambda 1)^\T x = \\
&\min_{w,\lambda} w^\T b + \lambda + \max_{x\geq 0} (e_i- A_S^\T w - \lambda 1)^\T x_S + (-A_{S^c}^\T w - \lambda 1)^\T x_{S^c}
\end{align*}
If \eqref{Second_eq}, $x_{S^c}=0$ and if \eqref{First_eq}, $x_S>0$ by complementary slackness, we thus have support recovery.
%. If \eqref{First_eq} holds, then $x_S$ can have arbitrary values - together the two conditions thus guarantee support recovery. 
