...
The problem of estimating traffic demands between areas or along routes within a city using traffic counts on links in a network is an important problem in transportation planning. While speed data and travel time estimates are increasingly more available from commercial vendors (e.g. HERE Traffic, Waze), they do not provide the crucial information about traffic volumes (flows). Good traffic demand estimates improve a road network's efficiency, reliability, and safety by providing for a more effective use of its infrastructure. One of the main challenges of this problem is the shortage of data needed for management of transportation networks. Traffic volumes are measured by static sensors (e.g. loop detectors mounted into the roads, road-side radar) and these sensor points are typically sparse. The problem of traffic demand estimation has been studied extensively in the transportation literature and is also called the origin-destination (OD) matrix estimation problem. 

A natural way of approaching this kind of problem is compressed sensing, which is a signal processing technique for efficiently acquiring and reconstructing a sparse signal from underdetermined linear systems. In recent years, compressed sensing has attracted considerable attention in areas of applied mathematics, computer science, and electrical engineering, with broad applications to imaging, data processing in wireless sensor networks \cite{luo2009compressive}, cloud computing \cite{kung2011cloudsense}, cognitive radios \cite{tian2007compressed}, spectrum sensing \cite{polo2009compressive}, and IC Trojan detection \cite{gwon2012statistical}.

In this paper, we formulate the traffic demand estimation problem as a nonnegative least squares problem with linear constraints and investigate the use of sparse flow measurements to recover the sparse route splits among routes starting from each origin\footnote{Based on a problem formulation by Alexander Kurzhanskiy (Berkeley, EECS) on the subject of "Calculating route splits from sparse link flow data"}. The sparsity assumption is intuitive for the traffic setting because it is natural for the flows out of a particular origin in a city to be concentrated in a handful other locations, rather than uniformly spread out over all other locations. For instance, a highway during the morning commute hours may have most of its flow concentrated on routes that flow towards key business centers of a nearby city. There may of course be a small amount of flow between all points, and we may model that small perturbation as additive noise in the usual compressed sensing methodology.

Our problem differs from most reconstruction problems in the literature in two main ways:
\begin{itemize}
\item Because the portions of flow out of each origin sums to 1, our problem formulation has built-in $\ell^1$ constraints. Compressed sensing largely depends on $\ell^1$ techniques to promote sparsity. Some recent work has introduced a reciprocal infinity-norm as another type of regularization that promotes sparsity and has also explored its application to convex clustering \cite{mert}. We use this work as a starting point.
\item The notion of multiple origins adds a block structure to the problem (multiple $\ell^1$ constraints). The previously mentioned work essentially addresses the single-block,
and we do not know of any existing compressed sensing techniques that address a block structure with $\ell^1$ constraints on each block. However, some previous work develops compressed sensing techniques for block structures without $\ell^1$ constraints.
\end{itemize}

Block-sparsity arises naturally in applications of multi-band signals and measurements of gene expression levels. Block-sparsity with $\ell^1$ constraints arise naturally in network propagation reconstruction applications, for example transportation, computer networks, and disease networks
...
\begin{equation}\label{assumption-lp}
...
\end{align}
implies that the solution $\hat x$ of \eqref{i-lp} coincides with  $x^*$. The complementary slackness condition for \eqref{l8-recovery-dual} is $x^\T(A^\T w + \lambda 1 - e_i) = 0$ and subtracting $(A_Sx_S)^\T w + \lambda x_S^\T 1 = x_S^\T e_i$ we get $(A_{S^c} x_{S^c})^\T w + \lambda x_{S^c}^\T = 0$ and with \eqref{Second_eq} we conclude $x_{S^c}= 0$. Because $x^*$ is the minimum cardinality solution (and so is $\hat x$), we then conclude that $\hat x = x^*$ by uniqueness.

It remains to show that the assumptions in the theorem imply \eqref{First_eq} and \eqref{Second_eq}. The dual of \eqref{assumption-lp} is
\begin{align*}
0<d^* &= \max_{u,v,\lambda} u -v +\min_{x, y\geq 0} (e_i^\T - \lambda^\T A_S - u1^\T) x + (\lambda^\T A_{S^c} + v1^T) y\\
&=\max_{u,v,\lambda} u-v \quad\text{such that}\quad e_i - A_S^T \lambda - u1 = 0 \text{ and } A_{S^c}^T \lambda + v1\geq 0.
\end{align*}
Here we used strong duality, which holds because $A_S x^* = b$

. This is because we know that the original $x^*$ satisfies the condition $\Phi_S x = f$. 
...
\end{proof}
=======
>>>>>>> 7da0296bac018cb18f295b1f965374b10da48930
...
\begin{theorem}[Restricted Isometry Property (RIP)]
